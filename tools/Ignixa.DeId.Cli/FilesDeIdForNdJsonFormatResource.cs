...
public class FilesDeIdForNdJsonFormatResource(
    IDeIdEngine engine,
    string inputFolder,
    string outputFolder,
    DeIdToolOptions options)
{
    ...
    private async Task ProcessNdJsonFile(string inputFile, string outputFile, CancellationToken cancellationToken)
    {
        ...
        await foreach (var result in _engine.DeidentifyManyAsync(lines, settings, cancellationToken).ConfigureAwait(false))
        {
            if (result.IsSuccess)
            {
                await outputWriter.WriteLineAsync(result.Value.DeidentifiedJson).ConfigureAwait(false);
                completedCount++;
            }
            else
            {
