...
var engine = provider.GetRequiredService<IDeIdEngine>();
...
if (result.IsSuccess)
{
    Console.WriteLine(result.Value.DeidentifiedJson);
...
```csharp
var result = await engine.DeidentifyAsync(resourceJson);

// Pattern matching
var output = result.Match(
    onSuccess: r => r.DeidentifiedJson,
...
if (result.IsSuccess)
{
    var de-identified = result.Value.DeidentifiedJson;
...
Control de-identification behavior per request using `RequestOptions`:
...
var result = await engine.DeidentifyAsync(resourceJson, options);
```
...
| `ValidateInput` | Validate input resources before de-identification | `false` |
| `ValidateOutput` | Validate de-identified output resources | `false` |
...
De-identification rules are defined in a JSON configuration file:
...
Array of de-identification rules. Each rule has:

| Field | Type | Description |
|-------|------|-------------|
| `path` | string | FHIRPath expression to select elements |
| `method` | string | De-identification method (see below) |
...
AES encryption for reversible de-identification.
...
**Use case:** When de-de-identification is required later.
...
Implement custom de-identification logic by creating a processor and registering it with DI:
...
De-identified resources are tagged with security labels:
...
| `--validate-input` | Validate input resources before de-identification | `false` |
| `--validate-output` | Validate de-identified output resources | `false` |
...
- Output: `./output/Patient/patient-123.json` (de-identified)

**NDJSON Format** (with `--bulk-data`):
- Input: `./input/export-patients.ndjson`
- Output: `./output/export-patients.ndjson` (de-identified, line-by-line)
