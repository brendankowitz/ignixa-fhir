...
| [canonical-search-type](investigations/canonical-search-type.md) | In Progress | Canonical (`url\|version#fragment`) search as a first-class value type and leaf lowering rule in `Ignixa.Search.Sql`, plus the canonical **resolution join** that `_include`/`_revinclude`/chaining need (e.g. `QuestionnaireResponse:questionnaire` → `Questionnaire.url` + `.version`) — a value join across two search-parameter indexes, unlike every existing id-based include |
