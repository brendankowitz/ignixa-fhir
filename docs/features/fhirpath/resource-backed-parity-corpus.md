...
| Backbone element handed to a leaf-typed parameter | 28 | **Not a converter gap; 27 of 28 now fixed** - one `Ignixa.Serialization` element-model defect (#454), described below. The 28th is R4B's own published expression, not the element model. |
...
The consequence was that six shipped search parameters indexed nothing on every version that ships them: `Encounter-location`, `Ingredient-manufacturer`, `MedicinalProductDefinition-contact`, `SubstanceDefinition-code`, `SubstanceDefinition-name` and `SubstanceSpecification-code`, plus four sites under two `SkippingElementNullOrEmptyInstanceType` signatures reached through the same class of mistyping (`Encounter.location.location` and `InventoryReport.inventoryListing.item.item`). Like the STU3 composites above, the API symptom was **an empty bundle with HTTP 200** - indistinguishable from "no matches".

A schema walk across all five generated providers finds **37 such qualified paths, 112 site-instances**, and they fall into two classes that matter very differently.

**33 paths / 93 sites have a backbone parent.** These are what this corpus measures. Their declared types are `CodeableConcept` (39 sites), `Reference` (20), `string` (15), `integer` (6), `Dosage` (4), `Money` (4), `Identifier` (3) and `CodeableReference` (2).

**Four have a datatype parent, and this corpus is blind to them.** The old heuristic's "last segment" fell back to the whole type name when there was no dot in it, so it fired on `Reference.reference`, `Expression.expression`, `id.id` and `Extension.extension` too — typing them as their own datatype rather than as the `string` the schema declares (`Extension.extension` was right by coincidence: it declares type `Extension`). **`Reference.reference` is by far the widest correction in the change** — it is on every reference in every resource on every version — and it is why `ResolveFunctionTests` re-pins from `Reference` to `string`. It moves no count here because no search parameter resolves a reference's own `reference` child, but it does move other consumers: it is why `FreeTextEdgeCaseStrategy` now needs an explicit structural-element exclusion, having previously been shielded by the mistyping.

The schema separates all of them from genuine recursion explicitly, and not by name: a genuinely recursive element carries a `contentReference` (`Questionnaire.item.item` declares `#Questionnaire.item`), while a false positive declares a real type and no `contentReference` at all.

Issue #454 keys the branch on that marker instead of on name equality, and resolves the `contentReference` to its actual target rather than assuming the target is the immediate parent. The second half matters independently. Commit 1 caught only the 19 paths where the child's name *also* matched the parent backbone's last segment; two things put a path in the remaining 76 (244 site-instances): the target is not the parent at all (`ExplanationOfBenefit.item.detail.subDetail.adjudication` points at an ancestor several levels up, `ValueSet.compose.exclude` at a sibling), or the target *is* the parent but the child is named differently (`OperationDefinition.parameter.part`). All of them used to fall through to the element's own unqualified name, which is not a valid FHIR type. All 307 `contentReference` declarations across the five schemas resolve, and `ContentReferenceResolutionTests` pins that rather than leaving it as a measurement someone once took.
