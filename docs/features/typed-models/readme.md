...
| [typed-primitive-values](investigations/typed-primitive-values.md) | Proposed | Whether `IElement.Value` should carry typed temporal values (Firely-style `P.Date`) instead of the wire `string`. Distinct axis from `primitive-fidelity`: **evaluation**, not serialization. Finds the fidelity-vs-typing tradeoff is false (Firely retains `OriginalParsedString`), that Ignixa already ships this pattern for `Quantity`, and that separating type from value has produced a 4-instance silent-empty-result bug class (one still open). Recommends the tactical boundary fix now, and — because `Ignixa.Abstractions` is packable and the repo is pre-1.0 (`release/0.6.41`) — resolving the contract's `DateTimeOffset or string` ambiguity before 1.0 freezes it. |
