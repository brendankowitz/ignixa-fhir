...
| [typed-primitive-values](investigations/typed-primitive-values.md) | Implemented — see [ADR-2610](adr-2610-typed-temporal-values.md) | Whether `IElement.Value` should carry typed temporal values (Firely-style `P.Date`) instead of the wire `string`. Distinct axis from `primitive-fidelity`: **evaluation**, not serialization. Found the fidelity-vs-typing tradeoff is false (Firely retains `OriginalParsedString`), that Ignixa already ships this pattern for `Quantity`, that separating type from value had produced a 4-instance silent-empty-result bug class, and — decisively — that FHIR's FHIRPath profile maps `date`/`dateTime`/`instant` to `System.DateTime`, not `System.String`, making the old `string` contract a conformance gap rather than a style choice. `IElement.Value` now returns `FhirTemporal` for temporals; `Quantity` is untouched. |
| [consolidate-handwritten-facades](investigations/consolidate-handwritten-facades.md) | Proposed (Phase 0 + Phase 0b implemented; Phase 1 in progress — Narrative/Extension merged) | Merges the 39 hand-written `*JsonNode` facades into the generated base via `partial class`, one type per resource — no rename-and-flip, no `ResourceTypeRegistry` dual-dispatch window. Phased: datatypes → contained resources → Application-layer facades → load-bearing `Bundle`/`OperationOutcome`/`Parameters` last. Bundle/Parameters/OperationOutcome now generated and version-tag-exempt on their base type (not yet merged); CapabilityStatement excluded from consolidation (STU3-only Application logic); Provenance/SearchParameter/StructureDefinition/StructureMap/ConceptMap/Composition to consolidate tagged, not exempt. Generator now has zero JsonNode fallbacks for the R4/R5 package (Reference, Resource, and contentReference elements all typed) — Phase 1 (the datatype merges) is fully unblocked. |
...
The three candidates above all address the **ergonomics** axis (how application code navigates a resource). [typed-primitive-values](investigations/typed-primitive-values.md) opens a fourth, orthogonal axis — the **evaluation** contract of `IElement.Value` itself — which applies regardless of which ergonomics option is chosen.

## Decision

Accepted and implemented across two ADRs: [adr-2606-typed-models](adr-2606-typed-models.md) (source-generated facades as zero-copy views over the JSON/`IElement` runtime — the core model) and [adr-2608-shared-base-models](adr-2608-shared-base-models.md) (shared base + per-version subclasses via inheritance, superseding 2606's one-assembly-per-version packaging). The generator is the tenth `ILanguage` in `codegen/Ignixa.Specification.Generators`, emitting checked-in source via a multi-version classification pass. Shipped for **R4 + R5**: a shared base layer in `Ignixa.Serialization` (namespace `Ignixa.Models`) plus opt-in per-version packages `src/Core/Models/Ignixa.Models.{R4,R5}` that inherit it. Identical types live once; version subclasses add only deltas; cross-version code targets the base (`As<R4.Patient>()` / `AsVersion(FhirVersion)`). `As<T>()` now also guards against cross-version misuse (`CompatibleFhirVersionsAttribute` on every generated facade; throws if a version-tagged node is reinterpreted through a facade that doesn't support that version). Proven by `test/Ignixa.Models.R4.Tests` (56 test cases) and `test/Ignixa.Models.Tests` (cross-version + classification + version-guard + enum/primitive coverage, 42 tests); the throwaway spike has been retired.

STU3 breadth is now open design: a spike measured that merging STU3 into the R4/R5 classification pass costs R4/R5 52% of their shared base (collateral demotion, not STU3 complexity). [adr-2609-stu3-classification-group](adr-2609-stu3-classification-group.md) (Proposed) recommends classifying STU3 as its own isolated group instead — full generated coverage, no shared base with R4/R5, enforced by the `As<T>()` guard above. R4B/R6 fan-out and a few edge cases (`contentReference`, enums-in-collections, alias ergonomics) remain open follow-ups, not yet investigated.

Orthogonal to ergonomics, [adr-2610-typed-temporal-values](adr-2610-typed-temporal-values.md) (Accepted, implemented) resolves the **evaluation** axis: `IElement.Value` now returns a typed `FhirTemporal` — wire literal plus parsed precision — for `date`/`dateTime`/`instant`/`time`, replacing the raw wire `string` and closing a FHIRPath spec-conformance gap. This applies regardless of which typed-model ergonomics option is chosen, since it changes the runtime `IElement`/FHIRPath contract, not the generated facades.
