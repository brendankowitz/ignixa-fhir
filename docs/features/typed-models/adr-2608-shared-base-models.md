...
- **Less duplication**: identical types live once. R4+R5 produced 71 base types (31 base-only, 40 subclassed); 30 elements were Incompatible across R4/R5 (e.g. `Attachment.size` `unsignedInt`→`integer64`, the `value[x]` choice families, and several value-set bindings that diverge under the same URL — see [adr-2609](adr-2609-stu3-classification-group.md)) and correctly split per version.
...
Implemented and verified for **R4 + R5** (2026-06-14, grown since): generator multi-version pass + classifier; base layer in `Ignixa.Serialization/Generated/Models`; `src/Core/Models/Ignixa.Models.{R4,R5}`; runtime `DomainResourceJsonNode` + `VersionedModelRegistry` + `AsVersion`. `All.sln` builds 0/0; `test/Ignixa.Models.R4.Tests` and `test/Ignixa.Models.Tests` (cross-version + classification + version-guard) pass — see [readme](readme.md) for current counts, which have grown past this ADR's original numbers and are tracked there rather than duplicated here; regen-drift guard (`build/check-typed-model-regen.{ps1,sh}`) clean.

Follow-ups: R4B/STU3/R6 fan-out; rename version entry-point classes for alias ergonomics; `contentReference` resolution; consolidate the hand-written `*JsonNode` facades into the generated base (separate migration — see [consolidate-handwritten-facades](investigations/consolidate-handwritten-facades.md)).
