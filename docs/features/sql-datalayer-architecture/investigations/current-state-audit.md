...
**Status**: Complete (describes the pre-Phase-0/1 baseline — see note below)
**Created**: 2026-07-11
**Updated**: 2026-07-11 (PR review pass)

> **Baseline snapshot notice**: every citation and line count below reflects the codebase at commit `96bfa953` (immediately before Phase 0/1 of `staged-query-compiler.md` executed). Phase 0 and Phase 1 have since shipped in the same PR that reviewed this doc (#328) and changed two of this doc's most load-bearing claims — see the inline notes marking exactly what changed, and `docs/superpowers/plans/2026-07-11-sql-datalayer-cleanup-phase-0-1.md`'s Post-Plan section for the full list of what shipped and what's still open.
...
`SearchExpressionQueryBuilder.ApplySearchExpressionAsync` (`Search/SearchExpressionQueryBuilder.cs:80-92`) dispatched on the *concrete Expression subtype* using a C# `switch` expression with `is`-style type patterns — not `expression.AcceptVisitor(...)`. Core already defines a full double-dispatch visitor contract (`Expressions/IExpressionVisitor.cs`, `DefaultExpressionVisitor.cs`, `ExpressionRewriter.cs`), used correctly elsewhere in Core (e.g. `CompartmentSearchRewriter`, `DateTimeEqualityRewriter`), but the SQL backend did not implement it. Practical consequence: adding a new `Expression` subtype to Core did not force the SQL backend to handle it at compile time — it fell through to `throw new NotSupportedException(...)` (`SearchExpressionQueryBuilder.cs:91`) at request time instead.

> **Fixed in Phase 1** (commit `d88bcf40`): `SearchExpressionQueryBuilder` now implements `IExpressionVisitor<SqlQueryContext, Task<IQueryable<ResourceEntity>>>` directly and dispatches via `expression.AcceptVisitor(this, context)`. A new `Expression` subtype added to Core today fails to compile in this class instead of throwing at request time. This paragraph is retained in past tense as the record of what motivated the fix.
...
`test/Ignixa.DataLayer.SqlEntityFramework.Tests/Search/` contains dedicated unit tests for `ChainedExpressionProcessor`, `IncludeProcessor`, `IterateProcessor`, `RevIncludeProcessor`, `NotReferencedSearchParameter`, and `ReferenceSearchParameter`. At baseline (`96bfa953`), no test file targeted `SearchParameterQueryGenerator` or `CompositeSearchParameterQueryGenerator` by name — the two largest and most duplicated files in the layer (2113 and 803 lines respectively) — with coverage presumably indirect via broader E2E search suites (`docs/features/e2e-testing/`), unconfirmed in this pass. No golden-SQL / generated-query-shape tests were found anywhere in the layer.

> **Fixed in Phase 0** (PR #328): `SearchParameterQueryGeneratorResourceLevelTests.cs` and `CompositeSearchParameterQueryGeneratorTests.cs` now target these two files directly, as characterization tests pinning down current comparator-dispatch behavior. The E2E-coverage question and the absence of golden-SQL tests remain open.
...
- `CompositeSearchParameterQueryGenerator.ApplyQuantityFilterAsync` (~lines 681-688): `ge → HighValue >= value`, `le → LowValue <= value` — range-*overlap* semantics, explicitly documented as deliberate in that method's own comments.
...
This is now the concrete "why" for Phase 2/3: a shared catalog or semantic predicate layer must make a *deliberate, spec-cited* choice about which semantics is correct, backed by characterization tests using `LowValue != HighValue` data, rather than mechanically unifying the two copies as if they'd always agreed. See `docs/superpowers/plans/2026-07-11-sql-datalayer-cleanup-phase-0-1.md`'s Post-Plan section for the full finding.
