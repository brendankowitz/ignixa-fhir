...
**Amended 2026-07-15 during execution — a real plan gap, discovered and correctly escalated by Task 4's first implementer attempt rather than papered over:**

`CompartmentSearchQueryGenerator` (Arm A, unmodified) does not read Task 3's seeded `dbo.SearchParam` catalog directly. It asks the **real** `CompartmentDefinitionManager` (loaded from the actual R4 compartment definitions) which resource types/codes belong to the Patient compartment, resolves each to a **real canonical FHIR URL** via `SearchParameterDefinitionManager`, then looks that URL up in `dbo.SearchParam` — no fallback, `null` on miss. Task 3's seeder paired 15 resource types with 10 synthetic `ignixa.dev` catalog codes via an `i % 10` rotation with no relationship to real compartment membership. Only 2 of the 15 seeded pairs (`Observation/subject`, `Encounter/patient`) turned out to be real, valid Patient-compartment associations; running Arm A as originally specified throws before any SQL executes, because its resolved `searchParamMap` is empty for the other 13. Full evidence (the pairing table, the exact exception, and the two rejected patch-around options) is in `.superpowers/sdd/task-4-report.md` in the executing worktree if still present, or in the commit history around this amendment.

**Resolution — re-seed using real associations, resolved via the real production managers, not a synthetic catalog:**

- [ ] **Step 0: Fix the seeded catalog to use real Patient-compartment associations**

Construct the real `CompartmentDefinitionManager` and `SearchParameterDefinitionManager` exactly as `SqlEntityFrameworkRepositoryFactory.cs` does for production DI (Task 4's first attempt already found and documented the exact construction — reuse it, don't re-derive it). Use them to enumerate the real (resourceType, code) associations the Patient compartment actually lists. Select roughly 10-15 spanning diverse resource types — reuse Task 3's original 15 resource types where they happen to have a real association, and substitute others where they don't, keeping the same skew shape (one hot association ≥500,000 rows, the rest in [100, 5000]).

Sync the **real** base FHIR search-parameter catalog into `dbo.SearchParam` via `SearchIndexReferenceDataCache.SyncSearchParametersToDatabase`, called with the **real** `ISearchParameterDefinitionManager` this time (not the `null!` placeholder Task 3's synthetic-catalog path used) — this is what makes the real canonical URLs resolvable by `SearchIndexReferenceDataCache.GetSearchParamIdAsync` during Arm A's own resolution.

Extend `CompartmentDataSeeder` (new method, or a parameterized variant of `SeedAsync` — controller/implementer's call, keep it minimal) to write `ReferenceSearchParam`/`Resource` rows keyed to these real, resolved `SearchParamId`s, preserving the skew shape. Do not delete or modify Task 3's original synthetic rows — they're simply inert to Arm A's real resolution and harmless to leave in place (the database has no teardown guard; leaving them is strictly simpler and lower-risk than trying to clean them up).

**Verify before proceeding to Step 1:** run Arm A's exact call (`CompartmentSearchQueryGenerator.GenerateCompartmentQueryAsync("Patient", "step0-patient", null, ct)`) against the re-seeded data and confirm it returns a non-empty, multi-CTE result instead of throwing. This is the acceptance gate for this step.

**Arms B and C must derive their `searchParamMap` from this same real resolution** — not from a separately-invented catalog. Concretely: call the same real `CompartmentDefinitionManager.TryGetSearchParams` + `SearchIndexReferenceDataCache.GetSearchParamIdAsync` resolution logic Arm A uses internally (using the real managers — this is reuse of real code, not the fake-manager approach the original brief correctly ruled out) to build the map Arms B and C need. This guarantees all three arms compare over an identical set of `(SearchParamId, ResourceTypeIds)` pairs — the whole point of the experiment.
