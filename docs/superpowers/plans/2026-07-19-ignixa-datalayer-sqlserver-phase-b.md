...
- **`Microsoft.SqlServer.DacFx`'s real API** (confirmed directly against Microsoft's own API reference and code samples, not assumed): `new DacServices(connectionString)`; `DacPackage.Load(Stream)` (also has `Load(string fileName)`); `void DacServices.Deploy(DacPackage package, string targetDatabaseName, bool upgradeExisting = false, DacDeployOptions options = default, CancellationToken? cancellationToken = default)` — **corrected per Task 4's empirical finding against a real SQL Server 2025 instance**: `upgradeExisting` only distinguishes "target database exists on the server" from "target database doesn't exist yet," not "empty" from "non-empty." `upgradeExisting: false` throws `DacServicesException` for *any* existing target, empty or not, so it cannot be the safety mechanism behind "existing tenant databases are never auto-touched" — `SchemaDeployer` uses `upgradeExisting: true` and relies solely on its own `IsDatabaseEmptyAsync` (`sys.tables`) check immediately before `Deploy` for that guarantee. Namespace: `Microsoft.SqlServer.Dac`. NuGet package id: `Microsoft.SqlServer.DacFx` (distinct from the namespace name — do not confuse the two in the `.csproj`). Not yet present in `Directory.Packages.props` — this plan adds it.
...
- `upgradeExisting: true` is required, not `false`: empirically (against a real, correctly-versioned SQL Server 2025 instance), DacFx's flag only distinguishes "target database exists on the server" from "target database doesn't exist yet," not "empty" from "non-empty." `upgradeExisting: false` throws `DacServicesException` for *any* pre-existing target — including the empty one this method is specifically designed to deploy to — because the target here always already exists by the time `Deploy` runs (created either by this same method's own `CreateEmptyDatabaseAsync` in dev mode, or pre-provisioned by ops). The actual safety mechanism enforcing "existing tenant databases are never auto-touched" is `IsDatabaseEmptyAsync`'s `sys.tables` check, called immediately before `Deploy` — if it's ever wrong (a bug elsewhere routes a non-empty database here), there is no DacFx-level backstop; this matches the old `DatabaseInitializer`'s own historical safety model (a single emptiness check right before acting, not a deeper engine-level guard).
