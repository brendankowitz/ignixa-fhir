...
- Resource-column predicates (`_id`/`_type`/`_lastUpdated`) are only extracted from the **top level of a conjunction** (a bare leaf, or a top-level `MultiaryExpression{And}`'s direct children) -- nested inside an `Or`, inside a `:not`, or inside a composite is out of scope for this plan and falls through to the generic dispatcher, which will throw `NotSupportedException` for `_id`/`_type` (wrong table) or a confusing mismatch for `_lastUpdated` -- acceptable for this increment; a comma-separated `_id=1,2,3` alternative is the most likely real-world case to hit this limit, deferred.
