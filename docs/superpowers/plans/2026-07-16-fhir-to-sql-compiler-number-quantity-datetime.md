...
**Post-task-2-review correction (found during execution, not caught while writing this plan):** the `Eq`/`Ne` formulas above were transcribed from `SearchParameterQueryGenerator.GenerateNumberQuery`'s `Equal`/`NotEqual` arms, which turned out to be **dead code** for the real comparator-driven search path — the live pipeline (`SearchValueExpressionBuilderHelper.GenerateNumberExpression`) actually widens the bound using `Ignixa.Search.Extensions.DecimalExtensions.GetPrescisionModifier()` (FHIR's implied-decimal-precision tolerance) before decomposing into `GreaterThanOrEqual`/`LessThanOrEqual` leaves. The real formulas are `Eq: LowValue >= (v-modifier) AND HighValue <= (v+modifier)`, `Ne: HighValue < (v-modifier) OR LowValue > (v+modifier)` — opposite comparison direction and precision-widened, not the raw `Eq: LowValue <= v AND HighValue >= v` this plan originally specified. This was caught in task 2's review, confirmed with the user, and fixed (commit `11b10fd4` in `worktree-fhir-to-sql-compiler-numeric-datetime`). **`NumericRangeComparison.Build`'s real, shipped signature is `Build(LeafContext context, SqlColumnRef lowColumn, SqlColumnRef highColumn, SearchComparator comparator, decimal value): Predicate`** — it now takes the raw `decimal` (not a pre-wrapped `SqlParameterRef`) and the `LeafContext` itself, since `Eq`/`Ne` need to compute and wrap two distinct widened bounds internally. **Before implementing task 3, read the real, current `src/Core/Ignixa.Search.Sql/Lowering/NumericRangeComparison.cs` directly — do not use this plan's original code sketches for `Eq`/`Ne` or the old 4-argument `Build` signature above, both are stale.**
