...
> **SUPERSEDED, kept as historical record only.** This plan's `Extension.SetValueUriRaw(string?)` design
> (a `public` instance mutator with no choice-clearing) shipped first, then was replaced twice more during
> PR review: first with `internal static Extension CreateWithRawValueUri(...)` (a factory, still no
> clearing but structurally single-use), then generalized into `internal void
> Extension.SetValueChoiceRaw(string valueElementName, string? value)` (clears any other `value[x]` key
> first, derived structurally from FHIR's `"value"` + PascalCase(type) wire convention — no R4/R5
> enumeration needed). The `Extension2` name below was also renamed to `Extensions`. See
> `docs/features/typed-models/investigations/consolidate-handwritten-facades.md`'s Phase 1 status section
> for the current, shipped design — every `SetValueUriRaw`/`Extension2` reference in the code blocks below
> reflects an earlier, abandoned iteration.
