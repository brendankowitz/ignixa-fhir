...
- `Lower.Run` gains a new optional parameter, **pairwise** exclusive with the existing keyset `page: PageSpec?` and with `top` (T-SQL forbids `TOP` and `OFFSET` in the same query, SQL Server error 10741) — offset-mode + `page` is a caller error, offset-mode + `top` is a caller error, throw on either, don't silently prefer one. **`page` + `top` together remains valid and unchanged** — that combination is keyset paging's own existing, correct call shape (`top` is keyset's page-size mechanism; `Lower.Run` already declares both as independent parameters, and existing keyset tests pass them together), not something this new mode's guard should touch.
- `Emit` renders `OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY` wherever the keyset boundary predicate and its `ORDER BY` already render today — the match-page CTE on an includes-bearing plan, or the single top-level `SELECT` on a plan with no includes (there is no match-page CTE to speak of in that case). The operative rule is "wherever the seek predicate and `ORDER BY` already live for this plan shape," not literally "inside a CTE" — on an includes-bearing plan specifically, that is the match-page CTE, and paging the outer `UNION ALL` instead would be wrong (it would page over match+include rows combined, not the match set alone, same reason keyset paging is already correctly scoped there). The existing `ORDER BY`-only-when-`Top is not null` gate extends to cover offset mode too (an `ORDER BY` inside a CTE is legal alongside `OFFSET`, so this composes cleanly).
...
- **The two-phase missing-value sort (`SortPhase.Valued`/`MissingPrimary`; per `SortSpec.cs:36-41`, "the executor drives the transition between the two") is genuinely new adapter logic — there is no existing reference implementation to read.** (Corrected from an earlier draft of this section, which incorrectly claimed `SqlEntityFrameworkSearchService` already implements this two-phase shape; it doesn't — the legacy service sorts in one query via Min/Max subqueries relying on SQL Server's default NULL-sorts-first behavior, a materially different approach, not a reference for this adapter's phase-driving loop.)
  ...
  **Fix:** run the `Valued` phase at the requested offset first. If it returns at least one row, the phase boundary is inside or past this page and `MissingPrimary`'s offset for this page is simply `0` — no further work needed. Only when `Valued` returns **zero rows** (the offset landed at-or-past the `Valued` total, and the boundary's exact location is now ambiguous — it could be anywhere from 0 rows short to arbitrarily far short) does the adapter run a `CountOnly` compile of the `Valued` phase (the compiler already supports `CountOnly` — no new mechanism) to learn the exact `Valued` total, then compute `MissingPrimary`'s offset as `max(0, requestedOffset - valuedTotal)`. Either way, `MissingPrimary`'s fetch limit is `Limit - (rows already returned by Valued)`, not the full requested `Limit` — running it at full `Limit` would over-fill the page whenever `Valued` contributed any rows at all. This fits entirely within the already-decided architecture (offset-mode compilation, existing `CountOnly`); it's a sequencing detail this design doc must state explicitly rather than leave for a plan-writer to discover the hard way.
