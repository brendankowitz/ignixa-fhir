...
- `Lower.Run` gains a new optional parameter, mutually exclusive with the existing keyset `page: PageSpec?` (supplying both is a caller error — throw, don't silently prefer one). Represents `(int Offset, int Limit)`.
- `Emit` renders `OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY` on the plan's outer/final `SELECT` when this mode is set, instead of the keyset boundary predicate.
...
- The two-phase missing-value sort (`SortPhase.Valued`/`MissingPrimary`; per `SortSpec.cs:36-41`, "the executor drives the transition between the two") is **not new work this decision introduces** — it was always going to be the adapter's responsibility as the "executor," under either pagination model. It was simply absent from the first draft. The adapter owns a small phase-driving loop: compile/execute the `Valued` phase, and only compile/execute `MissingPrimary` if the `Valued` phase's page didn't fill the requested count — same two-phase shape `SqlEntityFrameworkSearchService` already implements today for its own sort handling (read that implementation as the reference before designing the adapter's version).
- `SqlServerCompiledSearchService` decodes the incoming `SearchOptions.ContinuationToken` the same way `SqlEntityFrameworkSearchService` already does (existing decode logic, not new), translates it to the new offset-mode parameter, and mints outgoing tokens in the *same existing format* — the Application layer's contract is genuinely unchanged.
