...
## In scope: migrate `InMemoryIndex.SearchQueryInterpreter` off the lowerer

**Originally scoped as deferred future work; pulled into this implementation's scope.** Unlike Cosmos (`fhir-server`'s own backend, a different repo/organization boundary this design doesn't control), `InMemoryIndex` is in this repo and fully within reach. It likely carries its own instance of the untyped-value problem today (ad-hoc coercion from `object Value` + `FieldName`); consuming the typed IR directly fixes that the same way `Lower`'s Phase 3+ leaf rules will — pattern-match on `ISearchValue`, no coercion.

**Sequencing this correctly matters**, precisely because doing this removes the free regression guard described above: implement and land `LegacyExpressionLowerer` first, prove it correct via the tree-shape differential test (item 2), *then* migrate `InMemoryIndex` to bypass it — using the now-proven lowerer's behavior (not the pre-this-design old path) as the oracle for the behavioral test (item 5). Cosmos is unaffected either way and keeps consuming the lowered tree indefinitely, since it's out of this repo's control.
