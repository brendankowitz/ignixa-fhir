...
- **Migrate `InMemoryIndex.SearchQueryInterpreter` off `LegacyExpressionLowerer` to consume `SearchParameterPredicateExpression`/`CompositeComponentExpression` directly.** Unlike Cosmos (`fhir-server`'s backend, a different repo/organization boundary this design doesn't control), `InMemoryIndex` is in this repo and fully within reach. It likely has its own instance of the untyped-value problem today (ad-hoc coercion from `object Value` + `FieldName`), which consuming the typed IR directly would fix the same way `Lower`'s Phase 3+ leaf rules do — pattern-match on `ISearchValue`, no coercion.

  **Deliberately not part of this design**, for two reasons: (1) it's real new dispatch logic to write and test in `InMemoryIndex`, not the zero-new-code path the rest of this design achieves by reusing `SearchValueExpressionBuilderHelper`; (2) it would remove the cleanest regression guard this design gets for free — the differential test in *Testing* item 2 only works *because* `InMemoryIndex` doesn't change at all, so its old-path output is a stable oracle to diff against. Migrating it means that test's premise disappears and needs replacing with a behavioral equivalence check (same search results over the same data) instead of a tree-shape equivalence check.
