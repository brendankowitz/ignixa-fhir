...
2. **Differential test (intermediate safety net, not the final state)**: before `InMemoryIndex` is migrated (item 5 below), assert `LegacyExpressionLowerer`'s output is structurally identical (`ValueInsensitiveEquals`, already implemented on every `Expression` node) to what the *old* direct-construction path (`SearchValueExpressionBuilderHelper` invoked straight from the binder, pre-this-design) produces. This establishes `LegacyExpressionLowerer` is correct *before* anything is asked to depend on it being correct — including item 5's migration, which needs this proven first.
...
5. **Behavioral equivalence test, post-migration**: once `InMemoryIndex.SearchQueryInterpreter` consumes the typed IR directly (see below — now in scope, not deferred), the tree-shape differential test in item 2 no longer applies to it (there's no second "old" tree to diff against once the old path is gone). Replace it, for `InMemoryIndex` specifically, with a behavioral test: same search query against the same seeded in-memory data, before-and-after migration, asserting identical result sets — not identical trees.

## In scope: migrate `InMemoryIndex.SearchQueryInterpreter` off the external lowerer

**Originally scoped as deferred future work; pulled into this implementation's scope.** Unlike Cosmos (`fhir-server`'s own backend, a different repo/organization boundary this design doesn't control), `InMemoryIndex` is in this repo and fully within reach.

**Corrected 2026-07-15, after implementation — this section originally overclaimed the outcome.** The line above used to say consuming the typed IR directly would fix `InMemoryIndex`'s untyped-value coercion "the same way `Lower`'s Phase 3+ leaf rules will — pattern-match on `ISearchValue`, no coercion." **That premise was an unverified guess and turned out wrong for this code.** Implementation found that `VisitBinary`/`VisitString` are not the whole evaluation story: the real FHIR comparator semantics (date-range widening, quantity precision comparison, token system/code decomposition) live in `SearchValueExpressionBuilderHelper` — a ~380-line `ISearchValueVisitor` that historically ran once at parse time, not per-evaluation — with no independent characterization test suite for `SearchQueryInterpreter`'s own matching semantics to validate a fresh rewrite against. Hand-deriving that comparator math into a genuine `switch` on `ISearchValue`'s concrete type, with zero test oracle, on a non-default backend (`FileBasedSearchService`, reachable only via `TEST_USE_FILESYSTEM=true`) that already has severe pre-existing gaps (reference-typed and most composite searches already throw, before and after this work), was correctly judged too risky to do blind.

**What was actually delivered instead:** `SearchQueryInterpreter.VisitSearchParameterPredicate`/`VisitCompositeComponent` reconstruct the old field-level shape via the identical `SearchValueExpressionBuilderHelper.Build(...)` call `LegacyExpressionLowerer` makes, then dispatch through this class's own unmodified `VisitBinary`/`VisitString`. **The untyped-value coercion was relocated inline (per-predicate, at evaluation time), not eliminated.** This is genuinely equivalent by construction (both call sites are character-for-character the same helper invocation) and was proven so via a behavioral equivalence test — a real, valuable, low-risk outcome — but it is a narrower win than originally stated: `FileBasedSearchService` no longer needs an external lowering pass at its call sites, full stop. That is what "migrated off the lowerer" means for this class as shipped.

**The "pattern-match on `ISearchValue`, no coercion" goal remains genuinely available, but is not done and should not be assumed done by a future reader (in particular, Phase 3+ engineers modeling `Lower`'s own leaf-rule dispatch should not treat `SearchQueryInterpreter` as a working precedent for direct `ISearchValue` pattern-matching — it is the opposite, a precedent for reusing the lowerer inline to avoid that dispatch).** Re-scope it as its own future task, gated on first building a characterization test suite for `SearchQueryInterpreter`'s current matching semantics (there is none today) — attempting the rewrite without that scaffolding first would be repeating the exact risk this task correctly declined to take.

**Sequencing this correctly matters**, precisely because doing this removes the free regression guard described above: implement and land `LegacyExpressionLowerer` first, prove it correct via the tree-shape differential test (item 2), *then* migrate `InMemoryIndex` to bypass the external lowering step — using the now-proven lowerer's behavior (not the pre-this-design old path) as the oracle for the behavioral test (item 5). Cosmos is unaffected either way and keeps consuming the lowered tree indefinitely, since it's out of this repo's control.

**A second, previously-unknown production consumer was found during this sequencing, not anticipated by this design doc's original scope:** `SearchExpressionQueryBuilder`, the default SQL EF search backend, hand-dispatches on `Expression`'s runtime type via its own `switch` (not `IExpressionVisitor`) and broke independently once the binder's canonical output changed. Bridged via the same `LegacyExpressionLowerer` at its single entry point (a true bridge, not a migration — that backend is what the wider multi-phase compiler project eventually replaces). See `docs/superpowers/plans/2026-07-15-fhir-to-sql-compiler-phase2-semantic-ir.md`'s Task 6 for the full account.

  Recommended sequencing: land this design first, let the predicate IR prove itself against real traffic via the lowered path, then take on the `InMemoryIndex` migration as its own fast-follow — small blast radius (one file family), clear win (removes a live untyped-value bug class in Ignixa's own code), but wants its own test-plan update rather than inheriting this design's differential test as-is.
