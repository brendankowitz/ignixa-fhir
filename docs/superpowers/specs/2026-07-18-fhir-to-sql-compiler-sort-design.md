...
1. **A `CTE`'s own `ORDER BY` governs only which rows its `TOP` selects — it does not carry through to the statement that consumes the CTE.** So `cteMatchPage AS (SELECT TOP(n) ... ORDER BY ...)` correctly picks the right *rows*, but the final outer `SELECT` (the `UNION ALL` of match + include rows) needs its **own** `ORDER BY` to actually deliver them in order: `ORDER BY IsMatch DESC, <sort-key columns, NULL for include rows>, T1, Sid1` — extending the `IsMatch DESC` ordering Phase 7 already emits, not replacing it. When `Includes` is empty, there is no separate consuming statement — the plain `SELECT TOP(n) ... ORDER BY ...` is one self-contained statement and needs no second `ORDER BY`. **`cteMatchPage`'s own internal `ORDER BY` is itself conditional on `plan.Top` being set, not unconditional** — SQL Server rejects `ORDER BY` inside a CTE/derived table/subquery unless `TOP`/`OFFSET`/`FOR XML` is also present in that same `SELECT` (Msg 1033), so when `plan.Top` is `null`, `cteMatchPage` carries neither a `TOP` nor an `ORDER BY` of its own — it is still correctly ordered downstream by the outer `UNION ALL`'s own unconditional `ORDER BY`, described above, which is a plain top-level `SELECT` and is always legal regardless of `TOP`. The "every `TOP` needs an `ORDER BY`" invariant (§1.4, restated below) is about *pairing* — every `SELECT` that actually specifies a `TOP` must carry an `ORDER BY` in that same `SELECT` — not about forcing an `ORDER BY` onto a `SELECT` that has no `TOP` at all.
