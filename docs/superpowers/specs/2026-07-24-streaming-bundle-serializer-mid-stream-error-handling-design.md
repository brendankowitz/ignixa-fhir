...
- `WriteBundleIssues` — the R5 path — writes `issue.Severity`, `issue.Code`, and each `Location`/`Expression` string. §9 made `Location`/`Expression` tolerant (empty entries are skipped). `Severity`/`Code` were deliberately left as-is (a previous task's `GivenAThrowDuringThePrologue_...` test depends on an empty `Severity` throwing), so a follow-up round closed the asymmetry a different way: `SerializeWithPaginationAsync` now calls `ValidateBundleIssues` at the very top of the guarded region, before any writing, throwing on empty/whitespace `Severity`/`Code` there instead. The identical input now throws inside the guard on every FHIR version — via the prologue's `WriteBundleIssuesPreR5` path on R4, and via the new up-front validation on R5 — so `WriteBundleIssues`'s own writes are provably safe by the time they run.
