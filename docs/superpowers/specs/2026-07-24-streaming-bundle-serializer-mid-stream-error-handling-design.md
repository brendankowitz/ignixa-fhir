...
- **Writing zero bytes is not the same as touching nothing.** `Utf8JsonWriter` disposal calls `FlushAsync`, whose stream-backed path calls `_stream.FlushAsync()` **unconditionally — even with `BytesPending == 0`** — and an empty flush on `Response.Body` **starts the response** in Kestrel (headers sent, status defaults to 200). Because `await using` disposal runs during the unwind, *inside* the serializer and before the exception reaches `FhirExceptionMiddleware`, a plain `Reset()` commits a headers-only 200 and the middleware's `HasStarted` guard then correctly declines to write the real error. **`Reset(Stream.Null)` is what makes tier 1 work**: it re-points the writer so that the disposal flush lands on `Stream.Null` instead of the response. Reproduced over real Kestrel/TCP; invisible to `MemoryStream`-based tests, which see the byte count stay at zero either way.
...
**Tier 1 — nothing committed (`BytesCommitted == 0`).** `Reset(Stream.Null)` the main writer, then **rethrow**. Discarding the buffer is only half of it: redirecting the writer's destination is load-bearing, because the `await using` disposal that runs during the unwind flushes the writer's stream unconditionally, and that empty flush would start the response before the exception ever reached the middleware. With the redirect in place the disposal flush hits `Stream.Null`, the response is still unstarted, and `FhirExceptionMiddleware` sees `HasStarted == false` and produces a correct status code with a version-correct `OperationOutcome`. Virtually all `SerializeWithPaginationAsync` failures take this path.
...
**Those remaining writes are not inherently throw-free, and §9 plus a follow-up round make them so.** They pass caller-supplied strings to `FhirJsonWriter`'s validating writers, which `EnsureArg` against empty values (`FhirJsonWriter.cs:132-133`, `:143`), and one calls `BundleLink.GetRelationRaw()`, which can throw on a malformed JSON node:

- `WriteBundleIssues` — the R5 path — writes `issue.Severity`, `issue.Code`, and each `Location`/`Expression` string. §9 made `Location`/`Expression` tolerant (empty entries are skipped). `Severity`/`Code` were deliberately left as-is (a previous task's `GivenAThrowDuringThePrologue_...` test depends on an empty `Severity` throwing), so a follow-up round closed the asymmetry a different way: `SerializeWithPaginationAsync` now calls `ValidateBundleIssues` at the very top of the guarded region, before any writing, throwing on empty/whitespace `Severity`/`Code` there instead. `ValidateBundleIssues` is unconditional — it carries no `FhirVersion` check and runs before `WriteBundleIssuesPreR5` in the same `try` — so on **every** version the throw now originates there, uniformly, inside the guard and therefore recoverable. `WriteBundleIssues`'s and `WriteBundleIssuesPreR5`'s own `Severity`/`Code` writes are consequently provably safe by the time they run, and can no longer observe an empty value on this call path at all. One consequence worth knowing when reading the tests: `GivenAThrowDuringThePrologue_...` still passes with its assertions and text untouched, but it now exercises `ValidateBundleIssues` rather than the `WriteBundleIssuesPreR5` write its name and comment describe.
- `SerializeHistoryAsync`'s `WriteBundleLinks` writes `link.Url ?? string.Empty`; `BundleLink.Url` is `string?`, and §9 made empty/null URLs tolerant (the link is skipped instead of written unusable).
- `SerializeHistoryAsync`'s post-loop link filter also called `link.GetRelationRaw()` directly (to drop `"next"` when there is no further page), which reads `MutableNode["relation"]?.GetValue<string>()` and throws on a non-string node. Reaching it requires bypassing `SetRelationRaw(string)` via the low-level `SetProperty` escape hatch, so it is theoretical (§10 originally left it unclassified). A follow-up round closed it: link resolution (`"next"` suppression, empty-URL filtering, and the `GetRelationRaw()` read itself) was hoisted into a new `ResolveHistoryLinks` helper called inside the guard, right after the loop. The resolved links carry relations re-set via `SetRelationRaw` (through the existing `CreateLink` helper), so the post-guard `WriteBundleLinks` call that reads them back can never throw.

With those fixes, the post-guard region genuinely contains no throw source and Goal 1 holds on every path — not just "very slightly weaker than stated" (§10's original caveat), but closed.

`SerializeHistoryAsync`'s post-loop region likewise stays outside the guard, safe now that §9 and the follow-up round have landed. On its tier-2 error path the guard exits before that region, so the error bundle carries no `link` array — acceptable, since `bdl-18`'s self-link requirement is searchset-only and no history invariant requires links.
...
**Correction, established during implementation and independently verified against the pre-change source:** the paragraph above is written as though the never-throw contract already held. It did not. `WriteBundleFooterAsync` is called *outside* the existing try/catch, so a mid-entry corrupt write left the main writer at unbalanced depth, the catch captured the exception into `streamingException` without recovering the writer, and the unconditional footer call then hit `WriteEndArray()` against a writer that was never in array context — throwing `InvalidOperationException` **uncaught**, straight out of the method. The caller's catch would fire, return a 500, and skip `CompleteAsync()`. The background-task leak this section warns about was therefore **already reachable** before any of this work, by a different route. Per-entry buffering is consequently the change that *establishes* this method's never-throw contract, not one that merely protects it.
...
**Closed by a follow-up round** (a final whole-branch review flagged both as small, targeted fixes rather than scope creep):

- **`BundleLink.GetRelationRaw()` post-guard throw source — closed.** It reads `MutableNode["relation"]?.GetValue<string>()` and can throw if `relation` is not a string. `ResolveHistorySelfUrl` short-circuits at the first self link, so a malformed `relation` on a *later* link was not caught during resolution but threw in the post-guard link filter — the dispose-flush truncation path. Reaching it requires bypassing `SetRelationRaw(string)` via the low-level `SetProperty` escape hatch, which no production caller does, so it was theoretical, but §6's inventory of post-guard throw sources didn't classify it. Fixed by hoisting link resolution (`"next"` suppression, empty-URL filtering, and the `GetRelationRaw()` read) into the guard via a new `ResolveHistoryLinks` helper (§6). Covered by `StreamingBundleSerializerHistoryTests`: a zero-entry tier-1 case (empty stream, propagated exception) and a committed-entry tier-2 case (valid JSON, fatal outcome entry).
- **`WriteOperationOutcomeEntry`'s empty `selfUrl` on the history shape — closed.** Its history branches wrote `selfUrl` through the same empty-rejecting `WriteString`. `SerializeAsync` always passes `string.Empty`; harmless today because it never takes the `"history"` branch, but if `SerializeAsync` — dead code with zero callers — ever gained a history caller, this would have thrown *inside the catch*, replacing the original exception. Fixed with a cheap fallback to the same `"_history"` literal `ResolveHistorySelfUrl` uses when `selfUrl` is null or empty. Covered by `OperationOutcomeEntryShapeTests`.
