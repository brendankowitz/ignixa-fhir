...
internal sealed class AllowedResourceTypeFilter
{
    ...
    /// widening back to every type. Through <c>SearchCompiler</c> every name is already resolved (Resolve
    /// collects the allow-list), so the sentinel only arises for a direct <see cref="Lower.Run"/> caller
    /// that names a type it never put in the table.
    /// </summary>
