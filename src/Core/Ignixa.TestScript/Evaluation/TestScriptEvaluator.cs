    ...
    IFhirResourceValidator? validator = null) : ITestScriptActionVisitor
{
        ...
        ResourceJsonNode? capabilityStatement = null)
    {
        ...
        if (!setupFailed)
        {
            foreach (var test in definition.Tests)
            {
                if (!TestScriptVersionCompatibility.IsCompatible(test.FhirVersions, fhirVersion))
                {
                    RecordSkippedTest(recorder, test,
                        TestScriptVersionCompatibility.MismatchReason(test.FhirVersions, fhirVersion));
                    continue;
                }
