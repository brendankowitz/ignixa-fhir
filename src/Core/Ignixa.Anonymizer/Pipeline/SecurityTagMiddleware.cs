...
public sealed class SecurityTagMiddleware(ILogger<SecurityTagMiddleware> logger) : AnonymizerMiddleware
{
    ...
    private static AppliedSecurityLabels BuildSecurityLabels(Dictionary<string, int> operationCounts)
    {
        ...
        {
            IsRedacted = HasOperation(operationCounts, AnonymizationOperations.Redact),
            IsAbstracted = HasOperation(operationCounts, AnonymizationOperations.Abstract),
            IsCryptoHashed = HasOperation(operationCounts, AnonymizationOperations.CryptoHash),
            IsEncrypted = HasOperation(operationCounts, AnonymizationOperations.Encrypt),
            IsPerturbed = HasOperation(operationCounts, AnonymizationOperations.Perturb),
            IsSubstituted = HasOperation(operationCounts, AnonymizationOperations.Substitute),
            IsGeneralized = HasOperation(operationCounts, AnonymizationOperations.Generalize)
        };
    }
