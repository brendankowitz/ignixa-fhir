...
/// <summary>
/// Handler that executes de-identification processors for matched rules.
/// Resolves processors via keyed DI services and applies them to matched elements.
/// </summary>
internal sealed class ProcessorHandler : DeIdPipelineHandler
{
    private readonly Dictionary<string, IDeIdProcessor> _processorCache;
        ...
        ILogger<ProcessorHandler> logger)
    {
        ...
        _processorCache = new Dictionary<string, IDeIdProcessor>(StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var rule in options.Value.Rules)
        {
            ...
            {
                var processor = serviceProvider.GetKeyedService<IDeIdProcessor>(method);
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var matchedRule in context.MatchedRules)
        {
            ...
            // Use cached processor, or resolve at runtime for dynamically loaded configurations
            if (!_processorCache.TryGetValue(method, out var processor))
            {
                processor = _serviceProvider.GetKeyedService<IDeIdProcessor>(method);
                if (processor is not null)
                {
                    _processorCache[method] = processor;
                }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            return Result<bool>.Failure(new DeIdError(
                "PROCESSOR_ERROR",
