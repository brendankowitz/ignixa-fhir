...
/// </summary>
public class IpsGeneratorHandler : IRequestHandler<IpsGeneratorQuery, IpsGeneratorResult>
{
    private readonly IIpsGeneratorService _generatorService;
    private readonly ILogger<IpsGeneratorHandler> _logger;

    public IpsGeneratorHandler(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var ipsBundle = request.PatientId is not null
            ? await _generatorService.GenerateIpsAsync(
                request.PatientId,
                request.Profile,
                cancellationToken)
            : await _generatorService.GenerateIpsByIdentifierAsync(
