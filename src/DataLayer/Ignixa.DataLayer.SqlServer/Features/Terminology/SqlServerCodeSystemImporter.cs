...
#pragma warning restore CA2100
    ...
    /// procedure change because the procedures own their transaction. Best effort: if this statement fails,
    /// the caller's failure handling records <c>Failed</c> (retried on the next load); if that write also
    /// fails, the row stays at the procedure's <c>Completed</c> and the drop is visible only in the logs.
    /// </summary>
