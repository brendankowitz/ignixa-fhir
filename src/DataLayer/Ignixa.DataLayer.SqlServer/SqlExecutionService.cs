...
public sealed class SqlExecutionService : ISqlExecutionService
{
    ...
    {
        // Shares TenantConnectionStringResolver with SchemaDeployer/SchemaVersionResolver/the upgrade
        // CLI rather than resolving inline: it also applies the system partition's (tenant 0)
        // connection-string inheritance, which the terminology, package-resource and event-store
        // repositories depend on -- they address partition 0 directly. Resolving inline here would
        // throw for those callers on a correctly-configured single-tenant deployment.
