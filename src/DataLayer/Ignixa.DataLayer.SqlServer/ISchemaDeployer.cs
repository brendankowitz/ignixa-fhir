...
public interface ISchemaDeployer
{
    ...
    /// behind and the pending diff is provably safe to auto-apply (no item is flagged by
    /// SqlPackage/DacFx as a DataIssue -- see DeployReportClassifier). Throws if the tenant is
    /// behind and the diff is NOT auto-safe -- the caller must use the operator-triggered CLI
    /// path instead. No-ops if the tenant is already current.
    /// </summary>
