...
- **Migration Path**: Allows migration from microsoft/fhir-server to Sparky without data migration
...
The following tables from the legacy schema are **NOT** implemented in Sparky:
- **TaskInfo** - We have a better async processing plan (ADR-2502+)
- **JobQueue** - Replaced with our async architecture
- **ReindexJob** - Handled differently in Sparky
- **EventLog** - Using different logging approach
- **WatchdogLeases** - Not needed in Sparky architecture
...
### Integration with Sparky Architecture

```
┌─────────────────────────────────────────────────┐
│ Sparky.Api (ASP.NET Core)                      │
...
└────────────────┬────────────────────────────────┘
                 │
┌────────────────▼────────────────────────────────┐
│ Sparky.DataLayer.LegacySqlEF (This Project)    │
...
dotnet test test/Sparky.DataLayer.LegacySqlEF.Tests/ --filter Category=Integration

# Unit tests (no database required)
dotnet test test/Sparky.DataLayer.LegacySqlEF.Tests/ --filter Category=Unit
```
...
- **Sparky Architecture**: ../docs/architecture/overview.md

## License

MIT License - Same as Microsoft FHIR Server and Sparky project
