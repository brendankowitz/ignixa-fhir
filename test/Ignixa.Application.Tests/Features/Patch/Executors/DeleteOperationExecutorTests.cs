...
public class DeleteOperationExecutorTests
{
    ...
    {
        var resource = new ResourceJsonNode { ResourceType = "Patient", Id = "123" };
        ((IMutableJsonNode)resource).MutableNode["name"] = JsonNode.Parse(@"[{""family"":""Doe""},{""family"":""Smith""}]");
    ...
    {
        var resource = new ResourceJsonNode { ResourceType = "Patient", Id = "123" };
        ((IMutableJsonNode)resource).MutableNode["name"] = JsonNode.Parse(@"[{""given"":[""John"",""Michael""]}]");
    ...
    public async Task GivenBooleanProperty_WhenDeleting_ThenPropertyIsRemoved()
    {
        ...
        Assert.Null(((IMutableJsonNode)result).MutableNode["active"]);
    }
}
