...
public class TestScriptRegistryTests
{
    ...
    [Fact]
    public void GivenIdInDifferentCase_WhenLookingUp_ThenResolvesSameEntry()
    {
        // Arrange
        using var dir = new TempTestsDirectory();
        dir.WriteScript("Search/PatientSearch.json", TempTestsDirectory.ValidScriptJson("PatientSearch"));
        ...
        // Assert
        registry.TryGet("patientsearch").ShouldBeSameAs(registry.TryGet("PatientSearch"));
        registry.TryGet("crud/basic").ShouldBeSameAs(registry.TryGet("CRUD/Basic"));
    }
}
