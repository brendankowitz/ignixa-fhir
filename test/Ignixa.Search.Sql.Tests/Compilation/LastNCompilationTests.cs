...
public class LastNCompilationTests
{
    ...
    public async Task GivenAFilteredLastNRequest_WhenCompiled_ThenEmitsExactGroupingAndTieInclusiveRankingAfterTheCandidateSet()
    {
        ...
        compiled.Sql.ShouldContain("MIN(RootNodeId) AS CodeGroupId");
    ...
    [Fact]
    public void GivenLastNOptions_WhenCompiled_ThenEmitsOneReadOnlyStatementOverExistingTables()
    {
        // Arrange
        ...
        // Assert
        TSqlScript script = Ast.SqlGrammar.Parse(sql).ShouldBeOfType<TSqlScript>();
        script.Batches.Count.ShouldBe(1);
        script.Batches[0].Statements.Count.ShouldBe(1);
        script.Batches[0].Statements[0].ShouldBeOfType<SelectStatement>().Into.ShouldBeNull();
        Ast.SqlGrammar.Count<VariableTableReference>(script).ShouldBe(0);
        Ast.SqlGrammar.AssertEveryReferencedCteIsDefined(sql);
        string[] tables = Ast.SqlGrammar.FindAll<NamedTableReference>(script)
            .Where(table => table.SchemaObject.SchemaIdentifier is not null)
            .Select(table => table.SchemaObject.BaseIdentifier.Value)
            .Distinct()
            .Order()
            .ToArray();
        tables.ShouldBe(["DateTimeSearchParam", "Resource", "TokenSearchParam", "TokenText"]);
        sql.ShouldContain("OPTION (MAXRECURSION 0)");
    }
    ...
    [Fact]
    public async Task GivenTheDefaultLastNPlan_WhenCompiledRepeatedly_ThenSqlAndParametersAreDeterministic()
    {
        var options = new LastNSearchOptions(new SearchOptions(), 1, CodeParameter, DateParameter);
        SearchPlan plan = await new SearchSqlCompiler(LastNResolver()).CreateLastNPlanAsync(options);

        CompiledSearch first = plan.Compile();
        CompiledSearch second = plan.Compile();

        second.Sql.ShouldBe(first.Sql);
        second.Parameters.ShouldBe(first.Parameters);
    }
