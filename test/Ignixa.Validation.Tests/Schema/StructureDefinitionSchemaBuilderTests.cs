...
public class StructureDefinitionSchemaBuilderTests
{
    ...
    #region Constraint Scoping Tests

    [Fact]
    public void GivenMedicationRequestSchema_WhenBuilding_ThenDoesNotIncludeExtensionConstraint()
    {
        // Arrange
        // MedicationRequest has an extension field, but ext-1 constraint should NOT apply
        // ext-1 only applies to Extension resource type
        var summary = _provider.Provide("MedicationRequest");

        // Act
        var schema = _builder.BuildSchema(summary!, _provider);

        // Assert
        // Check that ext-1 (Extension-specific constraint) is NOT in the schema
        var fhirPathInvariantChecks = schema.Checks.OfType<FhirPathInvariantCheck>().ToList();

        // ext-1 should not be present for MedicationRequest
        fhirPathInvariantChecks
            .Where(c => c.ConstraintKey == "ext-1")
            .Should()
            .BeEmpty("ext-1 constraint should only apply to Extension resource type, not MedicationRequest");
    }

    [Fact]
    public void GivenExtensionSchema_WhenBuilding_ThenIncludesExt1Constraint()
    {
        // Arrange
        var summary = _provider.Provide("Extension");

        // Act
        var schema = _builder.BuildSchema(summary!, _provider);

        // Assert
        // Extension resource should have ext-1 constraint
        var fhirPathChecks = schema.Checks.OfType<FhirPathInvariantCheck>().ToList();
        fhirPathChecks
            .Where(c => c.ConstraintKey == "ext-1")
            .Should()
            .NotBeEmpty("ext-1 constraint should be present for Extension resource type");
    }

    [Fact]
    public void GivenBundleSchema_WhenBuilding_ThenIncludesBundleSpecificConstraints()
    {
        // Arrange
        var summary = _provider.Provide("Bundle");

        // Act
        var schema = _builder.BuildSchema(summary!, _provider);

        // Assert
        // Bundle should have Bundle-specific constraints like bdl-5 and bdl-8
        var fhirPathChecks = schema.Checks.OfType<FhirPathInvariantCheck>().ToList();
        var bundleSpecificKeys = new[] { "bdl-5", "bdl-8" };

        foreach (var key in bundleSpecificKeys)
        {
            fhirPathChecks
                .Where(c => c.ConstraintKey == key)
                .Should()
                .NotBeEmpty($"{key} constraint should be present for Bundle resource type");
        }
    }

    [Fact]
    public void GivenPatientSchema_WhenBuilding_ThenDoesNotIncludeBundleConstraints()
    {
        // Arrange
        var summary = _provider.Provide("Patient");
        ...
        // Assert
        // Patient should NOT have Bundle-specific constraints
        var fhirPathChecks = schema.Checks.OfType<FhirPathInvariantCheck>().ToList();
        var bundleSpecificKeys = new[] { "bdl-5", "bdl-8" };
        ...
        foreach (var key in bundleSpecificKeys)
        {
            fhirPathChecks
                .Where(c => c.ConstraintKey == key)
                .Should()
                .BeEmpty($"{key} constraint should NOT be present for Patient resource type");
        }
    }

    [Fact]
    public void GivenResourceWithUniversalConstraints_WhenBuilding_ThenIncludesEle1()
    {
        // Arrange
        // ele-1 is a universal constraint (applies to all resources via AppliesTo empty array)
        var summary = _provider.Provide("Patient");
        ...
        // Assert
        // ele-1 should be present for Patient (universal constraint)
        var fhirPathChecks = schema.Checks.OfType<FhirPathInvariantCheck>().ToList();
        fhirPathChecks
            .Where(c => c.ConstraintKey == "ele-1")
            .Should()
            .NotBeEmpty("ele-1 is a universal constraint and should be present for all resources");
    }

    [Fact]
    public void GivenMultipleResourceTypes_WhenBuilding_ThenEachHasOnlyApplicableConstraints()
    {
        // Arrange
        var resourceTypes = new[] { "Patient", "Observation", "Bundle", "Extension" };
        ...
        // Assert
        // ext-1 should only be in Extension schema
        foreach (var schema in schemas)
        {
            var fhirPathChecks = schema.Checks.OfType<FhirPathInvariantCheck>().ToList();
            var hasExt1 = fhirPathChecks.Any(c => c.ConstraintKey == "ext-1");

            if (schema.ResourceType == "Extension")
            {
                hasExt1.Should().BeTrue("ext-1 should be present for Extension");
            }
            else
            {
                hasExt1.Should().BeFalse($"ext-1 should NOT be present for {schema.ResourceType}");
            }
        }
    }
