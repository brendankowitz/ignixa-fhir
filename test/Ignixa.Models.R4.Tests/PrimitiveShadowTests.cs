...
/// <summary>
/// A FHIR primitive spans the value key plus the optional <c>_name</c> shadow (id/extensions).
/// These tests prove the (parent, name) primitive wrapper reads and writes both keys and survives
/// byte-for-byte round-trips. Ported from the typed-models spike (PrimitiveShadowSpikeTests).
/// </summary>
public sealed class PrimitiveShadowTests
{
    ...
    {
        var patient = ResourceJsonNode.Parse(PatientWithShadowJson).As<Ignixa.Models.R4.Patient>();
