...
public class CallSubScenarioStateTests
{
    ...
    public void GivenCardiovascularVitals_WhenExecuted_ThenCorrectObservationsRecorded()
    {
        ...
        var heartRateObs = scenario.Observations.FirstOrDefault(obs =>
            ((IMutableJsonNode)obs).MutableNode["code"]!["coding"]![0]!["code"]!.GetValue<string>() == "8867-4");
        ...
        var o2SatObs = scenario.Observations.FirstOrDefault(obs =>
            ((IMutableJsonNode)obs).MutableNode["code"]!["coding"]![0]!["code"]!.GetValue<string>() == "59408-5");
    ...
    public void GivenInfectionMonitoringVitals_WhenExecuted_ThenTemperatureAndRespiratoryRateRecorded()
    {
        ...
        var tempObs = scenario.Observations.FirstOrDefault(obs =>
            ((IMutableJsonNode)obs).MutableNode["code"]!["coding"]![0]!["code"]!.GetValue<string>() == "8310-5");
        ...
        var rrObs = scenario.Observations.FirstOrDefault(obs =>
            ((IMutableJsonNode)obs).MutableNode["code"]!["coding"]![0]!["code"]!.GetValue<string>() == "9279-1");
    ...
    public void GivenSubScenarioWithDelay_WhenExecuted_ThenTimeAdvances()
    {
        ...
        var firstEncounterTime = DateTime.Parse(((IMutableJsonNode)scenario.Encounters[0]).MutableNode["period"]?["start"]?.GetValue<string>()!);
        var secondEncounterTime = DateTime.Parse(((IMutableJsonNode)scenario.Encounters[1]).MutableNode["period"]?["start"]?.GetValue<string>()!);
    ...
    public void GivenCustomSubScenario_WhenExecuted_ThenStatesApplied()
    {
        ...
        var hrValue = ((IMutableJsonNode)heartRate!).MutableNode["valueQuantity"]!["value"]!.GetValue<decimal>();
