...
public sealed class CSharpTypedModelLanguage : ILanguage
{
        ...
        string primaryVersion)
    {
        ...
        return RenderClass(
            ...
            compatibleVersions: classification.PresentInVersions);
    }
    ...
    /// <summary>
    /// Maps a generator version key (e.g. "R4", "STU3") to its <c>FhirVersion</c> enum member name.
    /// Explicit rather than a case transform because they don't all agree (STU3 -> Stu3) -- an unmapped
    /// version should fail generation loudly, not silently emit an enum literal that doesn't compile.
    /// </summary>
    private static string MapToFhirVersionEnumMember(string version) => version switch
    {
        "R4" => "R4",
        "R4B" => "R4B",
        "R5" => "R5",
        "R6" => "R6",
        "STU3" => "Stu3",
        _ => throw new ArgumentException($"Unknown FHIR version '{version}' -- add a FhirVersion enum mapping in {nameof(MapToFhirVersionEnumMember)}.", nameof(version)),
    };
