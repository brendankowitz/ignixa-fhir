---
name: adr-analyzer
description: Use this agent when the user needs to understand, implement, or verify work against Architecture Decision Records (ADRs). This includes:\n\n<example>\nContext: User is working on the FHIR server prototype and wants to ensure their implementation aligns with ADR-2501.\nuser: "I've just finished implementing the FileBasedFhirRepository. Can you verify it matches what's specified in ADR-2501?"\nassistant: "I'll use the adr-implementation-analyzer agent to review your implementation against the ADR-2501 specifications."\n<Task tool invocation with adr-implementation-analyzer>\n</example>\n\n<example>\nContext: User is planning the next phase of work and needs a detailed breakdown of tasks from an ADR.\nuser: "What are the specific implementation steps for the multi-tenancy investigation in ADR-2502?"\nassistant: "Let me use the adr-implementation-analyzer agent to extract and break down the implementation requirements from ADR-2502."\n<Task tool invocation with adr-implementation-analyzer>\n</example>\n\n<example>\nContext: User has completed a feature and wants to verify completeness against the ADR.\nuser: "I've implemented the Patient endpoints. Can you check if I've covered everything from the prototype phase?"\nassistant: "I'll use the adr-implementation-analyzer agent to verify your implementation against the ADR-2501 prototype requirements."\n<Task tool invocation with adr-implementation-analyzer>\n</example>\n\nProactively use this agent when:\n- The user mentions ADR numbers (e.g., "ADR-2501", "ADR-2500")\n- The user asks about implementation requirements, phases, or roadmap items\n- The user needs to verify completeness of work against architectural decisions\n- The user is planning work and needs task breakdowns from ADRs\n- The user asks about what's next or what remains to be done in a phase
model: sonnet
